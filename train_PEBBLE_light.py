        ...
        if self.labeled_feedback > 0:
            for epoch in range(self.cfg.reward_update):
                if self.cfg.label_margin > 0 or self.cfg.teacher_eps_equal > 0:
                    ...
                    train_acc = self.reward_model.trainer.train_soft_reward(
                        self.reward_model.data_handler.buffer_seg1,
                        self.reward_model.data_handler.buffer_seg2,
                        self.reward_model.data_handler.buffer_label,
                        self.reward_model.data_handler.capacity,
                        self.reward_model.data_handler.buffer_full,
                    )
                else:
                    self.reward_model.train()
                    train_acc = self.reward_model.trainer.train_reward(
                        self.reward_model.data_handler.buffer_seg1,
                        self.reward_model.data_handler.buffer_seg2,
                        self.reward_model.data_handler.buffer_label,
                        self.reward_model.data_handler.capacity,
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.reward in ['learn_from_preference', 'learn_from_score']:
                if self.cfg.reward_data_type == "pointcloud":
                    ...
                    if point_cloud is not None:
                        ...
                        reward_hat = self.reward_model.trainer.r_hat(pc)
                    else:
                        reward_hat = reward
                    # 计算完毕后恢复模型到训练模式
                    self.reward_model.train()
                    ...
                    # 使用状态-动作向量作为输入（纯 MLP）
                    self.reward_model.eval()
                    reward_hat = self.reward_model.trainer.r_hat(np.concatenate([obs, action], axis=-1))
                    self.reward_model.train()
                else:
                    # 使用图像作为输入
                    ...
                    self.reward_model.eval()
                    reward_hat = self.reward_model.trainer.r_hat(image)
                    self.reward_model.train()
            ...
            if self.step % self.cfg.save_interval == 0 and self.step > 0:
                ...
                self.reward_model.trainer.save_models(model_save_dir, self.step)
