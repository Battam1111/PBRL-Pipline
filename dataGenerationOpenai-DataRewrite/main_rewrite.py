...
项目入口文件：
  1. 从 dataGenerationOpenai-DataRewrite/rewriteData 目录加载需要处理的 JSON 数据集，
     例如 "cap3d_objaverse_sft_45k.json" 和 "gapartnet_sft_27k_openai.json"。
  ...
  4. 根据配置选择 Direct API 模式或 Batch API 模式调用 OpenAI 接口，
     并将结果合并输出到对应的结果文件中。
  5. 最终将改写结果与原始数据合并，生成新的 JSON 文件存储在输出目录中。
"""

import os
from config import DATA_REWRITE_DIR, DATA_FILES, REWRITE_OUTPUT_DIR, USE_BATCH_API, CHUNK_SIZE, OPENAI_API_KEYS
...
def process_dataset(file_name):
    ...
    jsonl_file = os.path.join(REWRITE_OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_input.jsonl")
    jsonl_mgr = RewriteJSONLManager(tasks)
    jsonl_mgr.create_jsonl(jsonl_file)
    
    # 调用 OpenAI API（支持 Batch API 或 Direct API 模式）
    if USE_BATCH_API:
        log("使用 Batch API 模式。")
        ...
        out_files = batch_manager.process_chunk_files(chunk_files, REWRITE_OUTPUT_DIR)
        final_out = os.path.join(REWRITE_OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_output_merged.jsonl")
        ...
        final_out = os.path.join(REWRITE_OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_direct_results.jsonl")
    ...
    # 合并改写结果到原始数据中，并输出新 JSON 文件
    output_json = os.path.join(REWRITE_OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_rewritten.json")
    jsonl_mgr.merge_results_into_dataset(final_out, original_data, output_json)
