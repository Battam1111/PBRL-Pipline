...
项目入口文件：
  1. 从 DATA_REWRITE_DIR 目录加载需要处理的 JSON 数据集，
     例如 "cap3d_objaverse_sft_45k.json"。
  ...
  4. 根据配置选择 Direct API 模式或 Batch API 模式调用 OpenAI 接口，
     并将结果合并输出到对应的结果文件中。
  5. 当 USE_BATCH_API=False 时，最终将改写结果与原始数据合并，生成新的 JSON 文件存储在输出目录中；
     当 USE_BATCH_API=True 时，则不生成最终的 rewritten JSON 文件（因为 Batch 模式下逻辑不同）。
  6. 所有输出文件均存放于带有当前时间戳的子文件夹中，便于断点续传及区分测试批次。
     合并时使用原始 JSON 数据文件的基本名（例如 "cap3d_objaverse_sft_45k"）构造 custom_id，
     以确保正确匹配改写结果。
"""
import os
import time
