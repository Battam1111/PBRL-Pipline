...
class Picker(ActionToolBase):
    def __init__(self, num_picker=1, picker_radius=0.05, init_pos=(0., -0.1, 0.), picker_threshold=0.005, particle_radius=0.05,
                 picker_low=(-0.4, 0., -0.4), picker_high=(0.4, 0.5, 0.4), init_particle_pos=None, spring_coef=1.1, **kwargs):
        """
    ...
    def set_two_picker_pos(self, picker_pos_array):
        assert(len(picker_pos_array) == 2)
        """ Caution! Should only be called during the reset of the environment. Used only for cloth drop environment. """
        shape_states = np.array(pyflex.get_shape_states()).reshape(-1, 14)
        for i in range(self.num_picker):
            shape_states[i][3:6] = picker_pos_array[i]
            shape_states[i][:3] = picker_pos_array[i]
        pyflex.set_shape_states(shape_states)

    def set_one_picker_pos(self, pos, index):
        """ Caution! Should only be called during the reset of the environment. Used only for cloth drop environment. """
        shape_states = np.array(pyflex.get_shape_states()).reshape(-1, 14)
        shape_states[index][3:6] = ppos
        shape_states[index][:3] = pos
        pyflex.set_shape_states(shape_states)

    def set_picked_particle(self, picker_idx, particle_idx):
        self.picked_particles[picker_idx] = particle_idx 
        positions = pyflex.get_positions().reshape(-1, 4)
        positions[particle_idx, 3] = 0
        pyflex.set_positions(positions)

    def in_camera(self, x, lower_bound=-0.325, upper_bound=0.325):
        if x.shape[1] != 3:
            raise ValueError("Array must have the second dimension of size 3")

        # Check the bounds for the specified elements in columns 0 and 2
        return np.all((lower_bound <= x[:, 0]) & (x[:, 0] <= upper_bound)) and \
            np.all((lower_bound <= x[:, 2]) & (x[:, 2] <= upper_bound))
    ...
    def step(self, action):
        ...
        if self.in_camera(new_particle_pos[:, :3]) and self.in_camera(new_picker_pos):
            self._set_pos(new_picker_pos, new_particle_pos)
        # else:
        #     print('constrain!')

class PickerPlace(Picker):
    ...
    def reset(self, init_picker_poses):
        for picker_pos in init_picker_poses:
            pyflex.add_sphere(self.picker_radius, picker_pos, [1, 0, 0, 0])
        pos = pyflex.get_shape_states()  # Need to call this to update the shape collision
        pyflex.set_shape_states(pos)
        ...
        for (i, picker_pos) in enumerate(init_picker_poses):
            shape_state[i] = np.hstack([picker_pos, picker_pos, [1, 0, 0, 0], [1, 0, 0, 0]])
        pyflex.set_shape_states(shape_state)
        self.particle_inv_mass = pyflex.get_positions().reshape(-1, 4)[:, 3]

    def step(self, action):
        """
        action: Array of pick_num x 2. For each picker, the action should be [x, z]. 
        The picker will move the pick/drop state while moving towards x, z.
        the picker will move at a fixed height. 
        """
        total_steps = 0
        # import pdb; pdb.set_trace()
        ...
        # lift up 0.05
        actions = [np.array([0, 0.006, 0, 1] * self.num_picker) for _ in range(10)]
        for a in actions:
            super().step(a)
            pyflex.step()
            self.env.get_image()
        ...
        curr_pos = np.array(pyflex.get_shape_states()).reshape(-1, 14)[:, [0, 2]]
        end_pos = action
        ...
        for i in range(int(min(num_step, 300))):  # The maximum number of steps allowed for one pick and place
            ...
            # import pdb; pdb.set_trace()
            super_action = np.hstack([delta[:, 0].reshape(delta.shape[0], 1), 
                                      np.zeros((delta.shape[0], 1)), 
                                      delta[:, 1].reshape(delta.shape[0], 1), 
                                      np.ones((delta.shape[0], 1))])
            super().step(super_action)
            pyflex.step()
            total_steps += 1
            if self.env is not None and self.env.recording:
                self.env.video_frames.append(self.env.render(mode='rgb_array'))
