...
def init_model(args):
    ...
    # 加载模型时指定 torch_dtype=torch.bfloat16
    model = PointLLMLlamaForCausalLM.from_pretrained(
        model_name, 
        low_cpu_mem_usage=False, 
        use_cache=True,
        torch_dtype=torch.bfloat16  # 添加此行以支持bfloat16
    ).cuda()
...
def start_conversation(args, model, tokenizer, point_backbone_config, keywords, mm_use_point_start_end, conv):
    ...
    while True:
        ...
        def confirm_point_cloud(input_choice, object_id_input, point_cloud_input, chatbot, answer_time, conv):
            ...
            # 改成bfloat16
            point_clouds = torch.from_numpy(point_clouds).unsqueeze_(0).to(torch.bfloat16).cuda()
        ...
        def answer_generate(history, answer_time, point_clouds, conv):
            if point_clouds is None:
                ...
                # 不能改成bfloat16
                # input_ids = torch.as_tensor(inputs.input_ids).to(torch.bfloat16).cuda()
